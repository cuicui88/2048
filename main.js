	...
	for (var i = 0; i <4; i++) {
		for (var j = 0; j < 4; j++) {
			...
			if(board[i][j]==0){
				...
				number_cell.css('background-color',get_number_background_color(board[i][j]));/////待写
				number_cell.css('color',get_number_color(board[i][j]));////待写
...
function generate_one_number(){
	...
	//找到第50次还没有找到没有数字的位置时，在最前面没有数字的地方产生数字
	if(time==50){
		for(var i=0;i<4;i++){
			for(var j=0;j<4;j++){
				if(board[i][j]==0){
...
function prepare_for_mobile(){
	if(document_width >500){
		gird_contaoner_width = 500;
		cell_side_length = 100;
		cell_space = 20;
	}
	$('gird_container').css('width',grid_container_width - 2*cell_space);
	$('gird_container').css('height',grid_container_width - 2*cell_space);
	$('gird_container').css('padding',2*cell_space);
	$('gird_container').css('border-radius',0.02*grid_container_width);
...
document.addEventListener('touchend',function(event){
	...
	if(Math.abs(deltax)<0.3*document_width && (deltay)<0.3*document_width){
		return;
	}
	if($('#score').text == success_string){
		new_game();
